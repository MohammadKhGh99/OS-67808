...
public:
    ...
    int get_quantum_counter() const { return _quantum_counter; }

    void inc_quantum_counter() { _quantum_counter++; }

    int get_tid() const { return _tid; }

    int get_mutex_blocked() { return _mutex_blocked; }

    void set_mutex_blocked(int other) { _mutex_blocked = other; }

    void load_context() { siglongjmp(sig, 1); }

    int save_context() { return sigsetjmp(sig, 1); }
...
void setup(Thread *thread) {
    ...
    auto pc = (address_t) thread->entry_point;  //todo
...
int uthread_mutex_lock() {
    ...
    } else  //if (mutex != UNLOCKED)
    {
        if (mutex != running->get_tid()) {
            running->set_mutex_blocked(1);
            mutex_blocked.push_back(running);
            unblock_signals();
            return uthread_block(running->get_tid());
//            running->set_state(STATE::BLOCK);
//            mutex_blocked.push_back(running);
//            threads_switch(0); //todo change to 1 or what?
