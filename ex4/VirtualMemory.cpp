...
//	int firstResult, secondResult,
    ...
    if (depth != TABLES_DEPTH) {
        ...
        for (int i = 0; i < PAGE_SIZE; ++i) {
            ...
            {
                tmpEmptyPage = 0;
                frameFinding(table, index, path, emptyPage, array, depth + 1, value, PAGE_SIZE * page + i,
                             PAGE_SIZE * frame + i);
                if ((*emptyPage) == 0) {
                    continue;
                }
...
uint64_t physicalAddress(uint64_t virtualAddress)
{
    uint64_t frame = virtualAddress, offset = frame % (int) pow(2, OFFSET_WIDTH);
    bool flag = false;
    word_t path[TABLES_DEPTH + 1];
    path[0] = 0;
    for (int i = 0; i < TABLES_DEPTH; ++i) {
        int x = (int) pow(2, OFFSET_WIDTH * (TABLES_DEPTH - i));  // todo change to double or not?
        int j = (int) frame / x;
        frame %= x;
    ...
    return PAGE_SIZE * path[TABLES_DEPTH] + offset;
}
