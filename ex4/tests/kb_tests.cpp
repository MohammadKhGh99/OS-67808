 ...
 **/
 // This is based on last year's flow example, I modified the test slightly to fit
// the victim choosing method from this year.
// You can folow the example from last year but do the eviction according to
// the current method, it's not a very fun process though :(.
TEST(FlowTests, FlowTest)
{
    ...
    // the virtual address 6 will map to physical address 14.
    PMwrite(14, 1337);
...
//    // the virtual address 31 will map to physical address 9 (see PDF why this is true)
    PMwrite(9, 7331);
...
//         std::cout << "VM is " << "Writing to " << 5 * i * PAGE_SIZE << " the value " << i << std::endl;
